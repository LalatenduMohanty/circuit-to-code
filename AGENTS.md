...
Behavioral guidelines to reduce common LLM coding mistakes. Inspired by
...
This repo is **Circuit to Code**: markdown lessons under `lessons/` plus a PDF builder in `src/circuit_to_code/`.

| Path | Purpose |
|------|---------|
| `lessons/` | Course modules and syllabus |
...
Lesson SVGs under `lessons/**/diagrams/` must be well-formed XML (UTF-8, valid `<svg>`). Avoid raw `<` in text nodes.
